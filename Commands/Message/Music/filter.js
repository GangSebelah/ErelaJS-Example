...
module.exports = {
    ...
    category: "Music",
    aliases: [],
    description: "Show current music filter",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (["reset", "clear"].includes(args[0])) {
                ...
                embed.setDescription(`The current music filter is **${filter}**`);
                return message.channel.send({ embeds: [embed] });
            }
        } else {
            embed.setDescription(`There is no music filter`);
            return message.channel.send({ embeds: [embed] });
        }
    }
}