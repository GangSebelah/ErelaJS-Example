...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter television on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "television") {
                await resetFilter(player, message);
                player.set("filter", false);

                embed.setDescription(`Filter television has been disabled`);
                return message.channel.send({ embeds:[embed] });
            } else {
                await resetFilter(player, message);
                await player.node.send(data);
                player.set("filter", "television");

                embed.setDescription(`Filter television has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
