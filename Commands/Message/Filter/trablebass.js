...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter treblebass on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "treblebass") {
                ...
                embed.setDescription(`Filter treblebass has been disabled`);
                ...
                embed.setDescription(`Filter treblebass has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
