...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter speed on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        const value = args[0];
        if (!value) {
            embedError.setDescription(`Please provide the number of speed`);
            return message.channel.send({ embeds:[embedError] });
        }
        if (isNaN(value)) {
        ...
        if (filter) {
            if (filter === "speed") {
                ...
                embed.setDescription(`Filter speed has been disabled`);
                ...
                embed.setDescription(`Filter speed has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
