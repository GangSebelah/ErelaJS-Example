...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter pitch on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        const value = args[0];
        if (!value) {
            embedError.setDescription(`Please provide the number of pitch`);
            return message.channel.send({ embeds:[embedError] });
        }
        if (isNaN(value)) {
        ...
        if (filter) {
            if (filter === "pitch") {
                ...
                embed.setDescription(`Filter pitch has been disabled`);
                ...
                embed.setDescription(`Filter pitch has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
