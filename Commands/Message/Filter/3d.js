...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter 3d on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "3d") {
                ...
                embed.setDescription(`Filter 3D has been disabled`);
                return message.channel.send({ embeds:[embed] });
            } else {
                await resetFilter(player, message);
                await player.node.send(data);
                player.set("filter", "3d");

                embed.setDescription(`Filter 3D has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
