...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter gaming on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "gaming") {
                ...
                embed.setDescription(`Filter gaming has been disabled`);
                return message.channel.send({ embeds:[embed] });
            } else {
                await resetFilter(player, message);
                await player.node.send(data);
                player.set("filter", "gaming");

                embed.setDescription(`Filter gaming has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
