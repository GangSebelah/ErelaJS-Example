...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter vaporwave on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "vaporwave") {
                ...
                embed.setDescription(`Filter vaporwave has been disabled`);
                ...
                embed.setDescription(`Filter vaporwave has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
