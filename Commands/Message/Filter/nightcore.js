...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter nightcore on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "nightcore") {
                ...
                embed.setDescription(`Filter nightcore has been disabled`);
                ...
                embed.setDescription(`Filter nightcore has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
