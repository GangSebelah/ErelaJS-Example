...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter bassboost on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "bassboost") {
                ...
                embed.setDescription(`Filter bassboost has been disabled`);
                ...
                embed.setDescription(`Filter bassboost has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
