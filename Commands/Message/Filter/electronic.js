...
module.exports = {
    ...
    category: "Filter",
    aliases: [],
    description: "Toggles filter electronic on/off",
    args: false,
    usage: [],
    examples: [],
    memberPermissions: [],
    ...
    async execute(message, args) {
        ...
        if (filter) {
            if (filter === "electronic") {
                ...
                embed.setDescription(`Filter electronic has been disabled`);
                return message.channel.send({ embeds:[embed] });
            } else {
                await resetFilter(player, message);
                await player.node.send(data);
                player.set("filter", "electronic");

                embed.setDescription(`Filter electronic has been enabled`);
                return message.channel.send({ embeds:[embed] });
            }
        } else {
            await player.node.send(data);
